...
const encodeBinaryNodeInner = ({ tag, attrs, content }, opts, buffer) => {
    // Tambahkan pengecekan untuk attrs
    attrs = attrs || {};
    ...
    const writeString = (str) => {  
        // Tambahkan pengecekan untuk str null/undefined
        if (!str) {
            pushByte(TAGS.LIST_EMPTY);
            return;
        }
    ...
    // Perbaikan utama: Tambahkan pengecekan untuk attrs sebelum Object.keys()
    const validAttributes = (attrs && typeof attrs === 'object') ? 
        Object.keys(attrs).filter(k => (typeof attrs[k] !== 'undefined' && attrs[k] !== null)) : 
        [];
